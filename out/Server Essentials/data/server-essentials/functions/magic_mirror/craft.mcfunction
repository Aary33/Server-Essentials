...
# Target owner
execute as @s at @s as @a if score @s id.L = @e[limit=1,sort=nearest,distance=0..0.1,type=!minecraft:player] thrower.L if score @s id.M = @e[limit=1,sort=nearest,distance=0..0.1,type=!minecraft:player] thrower.M run advancement grant @s until server-essentials:magic/symbiosis/craft_with_symbiosis
kill @s
